...
const BlogPage = () => {
  ...
  async function fetchComment() {
        try {
      setLoading(true);
      const { data } = await axios.get<Comment[]>(`${blog_service}/api/v1/blogs/${id}/comments`);
      setComments(data);
    } catch (error) {
      console.log(error);
    } finally {
  ...
  async function addComment() {
    try {
     ...
     const { data } = await axios.post<MessageResponse>(
        `${blog_service}/api/v1/blogs/${id}/comment`,
        { comment },
        {
          headers: {
 ...
 setLoading(true);
    ...
    if (confirm("Are you sure you want to delete this comment")) {
      try {
        ...
        const { data } = await axios.delete<MessageResponse>(
          `${blog_service}/api/v1/blogs/comment/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  async function saveBlog() {
    ...
    try {
      setLoading(true);
      const { data } = await axios.post<MessageResponse>(
        `${blog_service}/api/v1/blogs/${id}/save`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
